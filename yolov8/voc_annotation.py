...
def convert_annotation(image_id, list_file):  
    # 修改路径，移除年份  
    in_file = open(os.path.join(VOCdevkit_path, 'VOC2007/Annotations/%s.xml' % image_id), encoding='utf-8')  
...
if __name__ == "__main__":  
    ...
    if annotation_mode == 0 or annotation_mode == 2:  
        ...
        for image_set in VOCdevkit_sets:  
            # 修改路径，移除年份  
            image_ids = open(os.path.join(VOCdevkit_path, 'VOC2007/ImageSets/Main/%s.txt' % image_set),   
                            encoding='utf-8').read().strip().split()  
            # 修改输出文件名，移除年份  
            list_file = open('%s.txt' % image_set, 'w', encoding='utf-8')  
            for image_id in image_ids:  
                # 修改路径，移除年份  
                list_file.write('%s/VOC2007/JPEGImages/%s.jpg' % (os.path.abspath(VOCdevkit_path), image_id))  
                # 修改函数调用，移除年份参数  
