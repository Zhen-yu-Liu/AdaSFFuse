...
if __name__ == "__main__":
    ...
    #   结果输出的文件夹，默认为map_out
    #-------------------------------------------------------#
    map_out_path    = 'map_out_fusion'                                                                #TODO:2
...
# if __name__ == "__main__":  
#     map_mode        = 0
#     classes_path    = 'model_data/cls_classes.txt'               #TODO:1
#     # 我们将创建一个IoU阈值列表，用于计算不同IoU下的mAP  
#     IOU_THRESHOLDS = np.arange(0.5, 1.0, 0.05) 
# #    IOU_THRESHOLDS  = [0.5, 0.75]  # 分别计算mAP0.5和mAP0.75  
#     confidence      = 0.001  
#     nms_iou         = 0.5  
#     score_threhold  = 0.5  
#     map_vis         = False  
#     VOCdevkit_path  = 'VOCdevkit'  
#     map_out_path    = 'map_out_vis_1'  
...
#         print("Get predict result.")  
#         for image_id in tqdm(image_ids):  
#             image_path  = os.path.join(VOCdevkit_path, "VOC2007/JPEGImages_vis/"+image_id+".jpg")  
#             image       = Image.open(image_path)  
#             if map_vis:  
#                 image.save(os.path.join(map_out_path, "images-optional/" + image_id + ".jpg"))  
#             yolo.get_map_txt(image_id, image, class_names, map_out_path)  
...
#             print(f"\nCalculating mAP at IoU={iou_threshold}")  
#             results = get_map(iou_threshold, True, score_threhold=score_threhold, path=map_out_path)  
            
#             # 这里可以添加代码保存每个类别在不同IoU下的结果  
#             # 假设results包含每个类别的AP值  
#             if results and isinstance(results, dict):  
#                 print(f"AP at IoU={iou_threshold}:")  
#                 for class_name, ap in results.items():  
#                     print(f"{class_name}: {ap:.4f}")  
#         print("Get map done.")  

#     if map_mode == 4:  
#         print("Get map.")  
#         # 使用COCO工具箱计算mAP0.50:0.95，并获取每个类别的结果  
#         class_aps = get_coco_map(class_names=class_names, path=map_out_path, return_per_class=True)  
