...
def main():
    ...
    ori_img_folder='/imagefusion/test/MFF'
    metric_result = np.zeros((8))
    for img_name in os.listdir(os.path.join(ori_img_folder,"imageA_Y")):
            base_name = img_name.rsplit('_', 1)[0]  # 去掉 '_A.png' 部分
            ir = image_read_cv2(os.path.join(ori_img_folder,"imageA_Y", img_name), 'GRAY')
            # import pdb;pdb.set_trace()
            vi = image_read_cv2(os.path.join(ori_img_folder,"imageB_Y",  img_name), 'GRAY')
