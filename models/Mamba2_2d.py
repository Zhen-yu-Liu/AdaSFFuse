...
class Mamba2_2d(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        print('d_in_proj',d_in_proj,'d_in:',self.d_inner,'g:',self.ngroups, 'd:',self.d_state)
    ...
    def forward(self, u, H, W, seq_idx=None):
        ...
        #2D Convolution
        # print('xBC_shape:',xBC.shape, 'H:',H ,'W',W)
        xBC = xBC.view(batch, H, W, -1).permute(0, 3, 1, 2).contiguous()
        # print('xBC_shape:',xBC.shape, 'H:',H ,'W',W)
        # Spatial domain branch
        xBC0 = self.act(self.se(xBC))
        # Frequency domain branch
        xBC1 = self.fft_filter(xBC)
        # Dynamic Fusion
        # xBC = self.down(torch.cat([xBC0, xBC1], dim=1))
        xBC = xBC0 + xBC1
        # print('xBC1_shape:',xBC.shape, 'H:',H ,'W',W)
...
if __name__ == '__main__':
    ...
    height = 32
    width = 32
    model = Mamba2_2d(d_model).to(device) 
